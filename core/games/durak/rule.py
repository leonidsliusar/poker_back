...
class DurakRule(AbstractRule):
    ...
    def defend(self, card: AbstractCard, next_player: AbstractPlayer) -> AbstractCard | None:
        ...
        defend_card = None
        if not self.is_defended(card_from_hand):
    ...
    def is_defended(self, card: AbstractCard) -> bool:
        attack_card = self.turn_of_player[1]
        if self.is_trump(card) and not self.is_trump(attack_card):
            return True
        return card > attack_card
